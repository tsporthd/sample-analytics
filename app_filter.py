...
class AppCodeFilter(ABC):
    ...
    @abstractmethod
    def get_executive(self, app_code: str) -> str:
        """Get the executive for a given AppCode."""
        pass
...
class CSVAppCodeFilter(AppCodeFilter):
    ...
    def __init__(self):
        ...
        self.executive_mapping: Dict[str, str] = {}

    def load_allowed_appcodes(self, source: str) -> Set[str]:
        """Load allowed AppCodes and Executive mapping from CSV file."""
        try:
            with open(source, 'r', newline='', encoding='utf-8') as file:
                ...
                for row in reader:
                    if 'AppCode' in row and row['AppCode'].strip():
                        app_code = row['AppCode'].strip()
                        appcodes.add(app_code)
                        if 'Executive' in row and row['Executive'].strip():
                            executive_mapping[app_code] = row['Executive'].strip()

                self.allowed_appcodes = appcodes
                self.executive_mapping = executive_mapping
    ...
    def get_executive(self, app_code: str) -> str:
        """Get the executive for a given AppCode."""
        return self.executive_mapping.get(app_code.strip(), '')
